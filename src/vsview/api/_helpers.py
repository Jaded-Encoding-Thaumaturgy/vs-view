...
class OutputMetadata(DefaultUserDict[str, dict[int, str]]):
    def _hash_key(self, key: str | PathLike[str]) -> str:
        from ..app.utils import path_to_hash

        return path_to_hash(key)

    def __getitem__(self, key: str | PathLike[str]) -> dict[int, str]:
        return super().__getitem__(self._hash_key(key))

    def __setitem__(self, key: str | PathLike[str], value: dict[int, str]) -> None:
