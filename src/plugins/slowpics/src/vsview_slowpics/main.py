...
class SlowPicsPlugin(WidgetPluginBase[GlobalSettings], IconReloadMixin):
    ...
    def __init__(self, parent: QWidget, api: PluginAPI) -> None:
        ...
        current_frames_row = QHBoxLayout(self)

        self.frames_dropdown = QComboBox(self)
        self.frames_dropdown.currentIndexChanged.connect(self._frame_selected)

        self.remove_manual_frame_btn = self.make_tool_button(IconName.MINUS, "Remove the current selected frame", self)
        ...
        self.add_manual_frame_btn = self.make_tool_button(IconName.PLUS, "Open a dialog to add frames manually", self)
        ...
        pictype_layout = QVBoxLayout(self)
        pictype_layout.addWidget(QLabel("Picture Types"))
        pictype_row = QHBoxLayout(self)
        self.i_frame = QCheckBox("I", self)
        self.p_frame = QCheckBox("P", self)
        self.b_frame = QCheckBox("B", self)
        ...
        pubnsfw_row = QHBoxLayout(self)
        self.public_check = QCheckBox("Public", self)
        self.public_check.setChecked(self.settings.global_.public_comp_default)
        self.nsfw_check = QCheckBox("NSFW", self)
        self.current_frame_check = QCheckBox("Include Current Frame", self)
        ...
        random_remove_layout = QHBoxLayout(self)

        random_layout = QVBoxLayout(self)
        random_layout.addWidget(QLabel("Random Frame Count", self))
        self.random_frames = QSpinBox(self)
        self.random_frames.setRange(0, 70)
        random_layout.addWidget(self.random_frames)

        remove_layout = QVBoxLayout(self)
        remove_layout.addWidget(QLabel("Remove After N days", self))
        self.remove_after = QSpinBox(self)
        ...
        light_dark_layout = QHBoxLayout(self)

        light_layout = QVBoxLayout(self)
        light_layout.addWidget(QLabel("Light Frames", self))
        self.light_frames = QSpinBox(self)
        self.light_frames.setRange(0, 10)
        light_layout.addWidget(self.light_frames)

        dark_layout = QVBoxLayout(self)
        dark_layout.addWidget(QLabel("Dark Frames", self))
        self.dark_frames = QSpinBox(self)
        ...
        meta_tag_row = QHBoxLayout(self)
        self.metadata_btn = QPushButton("Search TMDB", self)
        self.metadata_btn.clicked.connect(self._open_tmdb_search_popup)
        self.tags_btn = QPushButton("Select Tags", self)
        ...
        action_row = QHBoxLayout(self)
        self.get_frames_btn = QPushButton("Get Frames", self)
        self.extract_frames_btn = QPushButton("Extract Frames", self)
        self.upload_images_btn = QPushButton("Upload Images", self)
